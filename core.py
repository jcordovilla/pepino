...
class MessageAnalyzer:
    ...
    def _init_db(self):
        ...
        self.cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_messages_reference ON messages(referenced_message_id)"
        )
        self.cursor.execute(
    ...
    async def update_user_statistics(self, args: dict = None) -> str:
        ...
        try:
            ...
            for i, user in enumerate(users, 1):
                ...
                result += (
                    f"• Most Active: #{top_channel} ({channel_messages} messages)\n"
                )
                ...
                try:
                    ...
                    if user_messages:
                        user_concepts = await self.extract_concepts_from_content(
                            user_messages
                        )
                        if user_concepts:
                            ...
                            if meaningful_concepts:
                                ...
                                result += (
                                    f"• Main Topics: {', '.join(user_concepts[:3])}\n"
                                )
            ...
            avg_channels_per_user = (
                sum(user[3] for user in users) / len(users) if users else 0
            )
    ...
    def update_conversation_chains(self) -> Dict[str, Any]:
        ...
        for root_id, chain in chains.items():
            ...
            chain_stats["longest_chain"] = max(
                chain_stats["longest_chain"], chain_length
            )
        ...
        try:
            ...
            if daily_activity:
                try:
                    ...
                    plt.title(
                        "📈 Server Activity Trends - Past 30 Days",
                        fontsize=16,
                        fontweight="bold",
                        pad=20,
                    )
                    ...
                    if daily_stats:
                        ...
                        bars = plt.bar(
                            days_ordered, day_counts, color=colors, alpha=0.8
                        )
                        plt.title(
                        ...
                        for bar, count in zip(bars, day_counts):
                            if count > 0:
                                plt.text(
                                    ...
                                    bar.get_height() + max(day_counts) * 0.01,
                                    f"{count:,}",
                                    ha="center",
                                    va="bottom",
                                    fontsize=9,
                    ...
                    plt.savefig(
                        chart_path, dpi=300, bbox_inches="tight", facecolor="white"
                    )
                    plt.close()
            ...
            if daily_stats:
                ...
                sorted_days = sorted(
                    daily_stats.items(), key=lambda x: x[1], reverse=True
                )
            ...
            if hourly_stats:
                ...
                if total > 0:
                    ...
                    result += (
                        f"• 🌙 Night (00-05): {night:,} ({night/total*100:.1f}%)\\n"
                    )
...
class DiscordBotAnalyzer(MessageAnalyzer):
    ...
    async def analyze_topics_spacy(self, args: dict = None) -> str:
        ...
        try:
            ...
            if stats:
                ...
                replies_per_post = (
                    total_replies / original_posts if original_posts > 0 else 0
                )
                reaction_rate = (
                    (posts_with_reactions / total_msgs * 100) if total_msgs > 0 else 0
                )
                participation_rate = (
                    (active_contributors / total_members * 100)
                    if total_members > 0
                    else 0
                )
                ...
                result += (
                    f"• {reaction_rate:.1f}% reaction rate (messages with reactions)\n"
                )
                if total_members > 0:
                ...
                if earliest and latest:
                    try:
                        ...
                        if start_date == end_date:
                            ...
                            result += (
                                f"• Activity timeframe: {start_date} - {end_date}\n"
                            )
                    except:
                        pass
    ...
    async def extract_concepts_from_content(self, messages) -> List[str]:
        ...
        try:
            ...
            all_text = " ".join(
                [msg[0] for msg in messages if msg[0] and len(msg[0]) > 15]
            )
    ...
    async def resolve_channel_name(self, user_input: str, bot_guilds=None) -> str:
        ...
        try:
            ...
            for db_channel in db_channel_names:
                if (
                    user_input.lower() in db_channel.lower()
                    or db_channel.lower() in user_input.lower()
                ):
                    return db_channel
    ...
    async def get_user_insights(self, user_name: str) -> str:
        ...
        try:
            ...
            async with self.pool.execute(
                ...
                LIMIT 10
            """,
                (f"%{user_name.lower()}%", f"%{user_name.lower()}%"),
            ) as cursor:
            ...
            if recent_messages:
                try:
                    ...
                    try:
                        ...
                        print(
                            "spaCy model not found. Will use basic concept extraction."
                        )
                        user_concepts = await self.extract_concepts_from_content(
                            recent_messages
                        )
                        ...
                        def extract_complex_concepts(doc):
                            ...
                            for token in doc:
                                if token.dep_ == "nsubj" and token.pos_ in [
                                    "NOUN",
                                    "PROPN",
                                ]:
                                    subject_span = doc[
                                    ...
                                    if token.head.pos_ == "VERB":
                                        ...
                                        for child in verb.children:
                                            if child.dep_ in [
                                                ...
                                                obj_span = doc[
                                                    child.left_edge.i : child.right_edge.i
                                                    + 1
                                                ]
                                        ...
                                        if len(predicate_parts) > 1:
                                            ...
                                            if (
                                                len(full_concept.split()) >= 3
                                                and len(full_concept) > 15
                                            ):
                                                concepts.append(
                                                    full_concept.lower().strip()
                                                )
                            ...
                            for chunk in doc.noun_chunks:
                                ...
                                for token in chunk:
                                    for child in token.children:
                                        if child.dep_ == "prep":
                                            prep_phrase = doc[
                                                child.i : child.right_edge.i + 1
                                            ]
                                            extended_phrase += f" {prep_phrase.text}"
                                if (
                                    len(extended_phrase.split()) >= 3
                                    and len(extended_phrase) > 20
                                ):
                            ...
                            for i, token in enumerate(doc[:-2]):
                                if (
                                    ...
                                    compound = (
                                        f"{token.text} {doc[i+1].text} {doc[i+2].text}"
                                    )
                                    j = i + 3
                                    while (
                                        j < len(doc)
                                        and doc[j].pos_ in ["NOUN", "PROPN"]
                                        and j < i + 6
                                    ):
                        ...
                        combined_text = " ".join(
                            [msg[0] for msg in recent_messages if msg[0]]
                        )
                        ...
                        if cleaned_content and len(cleaned_content.split()) >= 10:
                            try:
                                doc = nlp(
                                    cleaned_content[:500000]
                                )  # Process user's content
                                ...
                                for pattern in tech_patterns:
                                    matches = re.findall(
                                        pattern, cleaned_content, re.IGNORECASE
                                    )
                                    for match in matches:
                                ...
                                print(
                                    f"Error in spaCy processing for {display_name}: {e}"
                                )
                        ...
                        technical_terms = [
                            term
                            for term, count in tech_counter.most_common(5)
                            if count >= 1
                        ]
                        business_concepts = [
                            concept
                            for concept, count in business_counter.most_common(5)
                            if count >= 1
                        ]
                        innovation_indicators = [
                            pattern
                            for pattern, count in innovation_counter.most_common(5)
                            if count >= 1
                        ]
            ...
            try:
                ...
                if daily_activity and len(daily_activity) > 1:
                    ...
                    if dates and message_counts:
                        ...
                        # Formatting
                        plt.title(
                            f"Daily Message Activity - {clean_user_name}",
                            fontsize=16,
                            fontweight="bold",
                            pad=20,
                        )
            ...
            if first_msg and last_msg:
                ...
                try:
                    ...
                    result += (
                        f"• First Message: {first_dt.strftime('%Y-%m-%d %H:%M')}\n"
                    )
            ...
            if channel_activity:
                ...
                for channel, count, avg_chars in channel_activity:
                    result += (
                        f"• #{channel}: {count} messages (avg {avg_chars:.0f} chars)\n"
                    )
                result += "\n"
            ...
            if user_concepts:
                ...
                for concept in user_concepts[:8]:
                    ...
                    formatted_concept = " ".join(
                        word.capitalize() for word in concept.split()
                    )
    ...
    async def get_channel_insights(self, channel_name: str) -> str:
        ...
        try:
            ...
            reaction_rate = (
                (posts_with_reactions / total_messages * 100)
                if total_messages > 0
                else 0
            )
            ...
            channel_amp = (
                (weekly_active / unique_human_users * 100)
                if unique_human_users > 0
                else 0
            )
            ...
            inactive_percentage = (
                (inactive_users / unique_human_users * 100)
                if unique_human_users > 0
                else 0
            )
            ...
            try:
                ...
                if member_result and member_result[0]:
                    ...
                    participation_rate = (
                        (unique_human_users / total_channel_members * 100)
                    ...
                    try:
                        ...
                        def extract_complex_topics(doc):
                                ...
                                if token.dep_ == "nsubj" and token.pos_ in [
                                    ...
                                    if token.head.pos_ == "VERB":
                                        ...
                                        if len(predicate_parts) > 1:
                                            ...
                                            if (
                                                len(full_topic.split()) >= 2
                                                and len(full_topic) > 10
                                            ):
                                                topics.append(
                                                    full_topic.lower().strip()
                                                )
                            ...
                            for i, token in enumerate(doc[:-1]):
                                if token.pos_ in ["NOUN", "PROPN", "ADJ"] and doc[
                                    i + 1
                                ].pos_ in ["NOUN", "PROPN"]:
                        ...
                        all_topics = []
                        combined_text = " ".join(
                            [msg[0] for msg in topic_messages if msg[0]]
                        )
                        cleaned_content = clean_content(combined_text)
                        ...
                        for topic, count in topic_counter.most_common(50):
                            if (
                                ...
                                and not topic.startswith(("i ", "you ", "we ", "they "))
                            ):
                                # Title case the topic for display
                                formatted_topic = " ".join(
            ...
            try:
                ...
                if daily_activity and len(daily_activity) > 1:
                    ...
                    if dates and message_counts:
                        ...
                        # Formatting
                        plt.title(
                            f"Daily Message Activity - {clean_channel_name}",
                            fontsize=16,
                            fontweight="bold",
                            pad=20,
                        )
            ...
            if contributors:
                ...
                for name, count, avg_chars in contributors:
                    result += (
                        f"• {name}: {count:,} messages (avg {avg_chars:.0f} chars)\n"
                    )
                result += "\n"
