...
AnalyzerType = Union[
    "ChannelAnalyzer", "UserAnalyzer", "TopicAnalyzer", "TemporalAnalyzer"
]
import json
...
class TemplateEngine:
    ...
    def __init__(
        ...
        analyzers: Optional[Dict[str, AnalyzerType]] = None,
        data_facade: Optional["AnalysisDataFacade"] = None,
    ...
    def render_template(
        self, template_name: str, messages: Optional[List[Dict]] = None, **kwargs
    ) -> str:
        """
        ...
        try:
            ...
            if messages:
                ...
                kwargs["all_message_text"] = " ".join(
                    msg.get("content", "") for msg in messages if msg.get("content")
                )
                kwargs["message_authors"] = list(
                    set(msg.get("author", "") for msg in messages if msg.get("author"))
                )

            return template.render(**kwargs)
        except Exception as e:
            logger.error(
                f"Error rendering template {template_name}: {str(e)}", exc_info=True
            )
            return f"❌ Error rendering template: {str(e)}"

    def render_string(
        self, template_string: str, messages: Optional[List[Dict]] = None, **kwargs
    ) -> str:
        """
        ...
        if self.analyzers:
            for name, analyzer in self.analyzers.items():
                ...
                if hasattr(analyzer, "get_available_users"):
                    self.env.globals[
                        f"get_available_users"
                    ] = analyzer.get_available_users
                if hasattr(analyzer, "get_available_channels"):
                    self.env.globals[
                        f"get_available_channels"
                    ] = analyzer.get_available_channels
    ...
    def _register_nlp_functions(self):
        ...
        if self.nlp_service:
            ...
            ] = self._analyze_messages_sentiment
            self.env.globals[
                "extract_message_concepts"
            ] = self._extract_message_concepts
    ...
    def _get_messages_by_channel(
        self, channel_name: str, limit: int = 100
    ) -> List[Dict]:
        """Get recent messages from a specific channel."""
        if not self.data_facade:
            return []
        try:
            return self.data_facade.message_repository.get_messages_by_channel(
                channel_name, limit
            )
        except Exception as e:
    ...
    def _get_messages_by_user(self, username: str, limit: int = 100) -> List[Dict]:
        ...
        if not self.data_facade:
            return []
        try:
            return self.data_facade.message_repository.get_messages_by_user(
                username, limit
            )
        except Exception as e:
    ...
    def _get_recent_messages(
        self, limit: int = 100, channel_name: Optional[str] = None
    ) -> List[Dict]:
    ...
    def _format_trend(
        self, trend: str, percentage: float, timeframe: str = None
    ) -> str:
    ...
    def _create_activity_chart(
        self, dates: List[str], counts: List[int], title: str = "Activity Over Time"
    ) -> str:
        """
    ...
    def _create_pie_chart(
        self, names: List[str], counts: List[int], title: str = "Distribution"
    ) -> str:
        """
    ...
    def _create_bar_chart(
        self, names: List[str], counts: List[int], title: str = "Activity Ranking"
    ) -> str:
        """
    ...
    def _create_wordcloud(
        self, text_data: Union[str, List[str]], title: str = "Word Cloud"
    ) -> str:
        """
