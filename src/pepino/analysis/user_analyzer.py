...
class UserAnalyzer:
    ...
    def analyze(
        self, username: str, days: Optional[int] = None, include_patterns: bool = True
        ...
        try:
            ...
            try:
                request = validate_user_analysis_input(
                    username=username, days=days, include_patterns=include_patterns
                )
            ...
            statistics = self._get_user_statistics_via_repository(
            ...
            channel_activity = self._get_user_channel_activity_via_repository(
                request.username, request.days
            )
            ...
            if request.include_patterns:
                time_patterns = self._analyze_time_patterns_via_repository(
                    request.username, request.days
                )

            # Create summary
            summary = self._create_user_summary(
                statistics, channel_activity, time_patterns
            )
    ...
    def analyze_enhanced(
        self, username: str, days: Optional[int] = None, include_semantic: bool = True
        ...
        try:
            ...
            statistics = self._get_enhanced_user_statistics_via_repository(
                username, days
            )
            ...
            channel_activity = self._get_enhanced_channel_activity_via_repository(
                username, days
            )
            ...
            if include_semantic:
                semantic_analysis = self._get_semantic_analysis_via_repository(
                    username, days
                )
            ...
            display_name = (
                metadata.get("display_name") or statistics.display_name or username
            )
            username_value = (
                metadata.get("author_name") or statistics.author_name or username
            )
            ...
            user_info = UserInfo(
                ...
                guilds=metadata.get("guilds", []),
                total_messages=metadata.get("total_messages")
                or statistics.message_count,
                unique_channels=metadata.get("unique_channels")
                or statistics.channels_active,
            )
    ...
    def _get_user_statistics_via_repository(
        self, username: str, days: Optional[int]
    ) -> Optional[LocalUserStatistics]:
        """Get basic user statistics using data facade pattern."""

        try:
            # Use data facade for repository access
            stats_data = self.data_facade.user_repository.get_user_message_statistics(
                username, days
            )
            ...
            if (
                total_messages > 0
                and stats_data["first_message"]
                and stats_data["last_message"]
            ):
                first_date = datetime.fromisoformat(
                    stats_data["first_message"].replace("Z", "+00:00")
                )
                last_date = datetime.fromisoformat(
                    stats_data["last_message"].replace("Z", "+00:00")
                )
    ...
    def _get_user_channel_activity_via_repository(
        self, username: str, days: Optional[int], limit: int = 20
    ) -> List[ChannelActivity]:
        """Get user's activity distribution across channels using data facade."""

        try:
            # Use data facade for repository access
            activity_data = self.data_facade.user_repository.get_user_channel_activity(
                username, days, limit
            )
    ...
    def _analyze_time_patterns_via_repository(
        ...
        try:
            ...
            hourly_data = self.data_facade.user_repository.get_user_hourly_patterns(
                username, days
            )
    ...
    def _get_user_metadata(self, username: str) -> Dict[str, Any]:
        ...
        try:
            if (
                not hasattr(self.data_facade, "user_repository")
                or self.data_facade.user_repository is None
            ):
                return {}
            metadata = self.data_facade.user_repository.get_user_metadata(
                username=username
            )
            return metadata or {}
        except Exception as e:
    ...
    def get_top_users(
        ...
        try:
            return self.data_facade.user_repository.get_top_users_by_message_count(
                limit, days
            )
        except Exception as e:
    ...
    def get_user_health(self, username: str) -> Dict[str, any]:
        ...
        try:
            # Get basic stats
            recent_stats = self.user_repository.get_user_message_statistics(
                username, days=7
            )
            overall_stats = self.user_repository.get_user_message_statistics(
                username, days=None
            )
            ...
            if channels <= 2:
                recommendations.append(
                    "User could benefit from engaging in more channels"
                )
    ...
    def get_user_channel_comparison(
        ...
        try:
            channel_activity = self.user_repository.get_user_channel_activity(
                username, days, limit=50
            )
            ...
            total_messages = sum(
                activity["message_count"] for activity in channel_activity
            )
            ...
            for activity in channel_activity:
                percentage = (
                    (activity["message_count"] / total_messages * 100)
                    if total_messages > 0
                    else 0
                )
                channels.append(
            ...
            if total_messages > 0:
                herfindahl = sum(
                    (activity["message_count"] / total_messages) ** 2
                    for activity in channel_activity
                )
    ...
    def _get_enhanced_user_statistics_via_repository(
        self, username: str, days: Optional[int]
    ) -> Optional[EnhancedUserStatistics]:
        """Get enhanced user statistics using data facade pattern."""

        try:
            # Use data facade for repository access
            stats_data = self.data_facade.user_repository.get_user_enhanced_statistics(
                username, days
            )
            ...
            try:
                ...
                if result and result["author_display_name"]:
                    ...
                    """
                    fallback_result = (
                        self.data_facade.user_repository.db_manager.execute_query(
    ...
    def _get_enhanced_channel_activity_via_repository(
        self, username: str, days: Optional[int], limit: int = 10
        ...
        try:
            # Use data facade for repository access
            activity_data = (
                self.data_facade.user_repository.get_user_channel_activity_enhanced(
                    username, days, limit
                )
            )
    ...
    def _get_time_patterns_via_repository(
        self, username: str, days: Optional[int]
    ) -> List[TimeOfDayActivity]:
        """Get time of day patterns using data facade."""

        try:
            # Use data facade for repository access
            time_data = self.data_facade.user_repository.get_user_time_of_day_patterns(
                username, days
            )

            time_patterns = []
            for period, count in time_data.items():
                pattern = TimeOfDayActivity(period=period, message_count=count)
    ...
    def _get_semantic_analysis_via_repository(
        self, username: str, days: Optional[int]
    ...
    def _get_user_topics_via_repository(
        self, username: str, days: Optional[int]
    ) -> List["TopicItem"]:
        ...
        try:
            ...
            if not user_messages or len(user_messages) < 5:
                logger.info(
                    f"Not enough messages for topic analysis for user {username}"
                )
                return []

            # Initialize topic analyzer
            topic_analyzer = TopicAnalyzer()
            topic_analyzer.data_facade = (
                self.data_facade
            )  # Pass data facade for consistency

            # Extract topics from user messages
            logger.info(
                f"Extracting topics from {len(user_messages)} messages for user {username}"
            )
    ...
    def _fallback_user_topics(
        ...
        try:
            ...
            logger.info(
                f"Fallback topic analysis found {len(topic_items)} topics for user {username}"
            )
            return topic_items
    ...
    def _get_user_messages_for_topics(
        self, username: str, days: Optional[int], limit: int
    ) -> List[str]:
        ...
        try:
            ...
            results = self.data_facade.user_repository.db_manager.execute_query(
                query, tuple(params)
            )
