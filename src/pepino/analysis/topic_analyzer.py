...
class TopicAnalyzer:
    ...
    def initialize(self) -> None:
        ...
        try:
            ...
            logger.info(
                "Please install with: poetry add bertopic umap-learn hdbscan transformers torch"
            )
            raise
    ...
    def _extract_domain_patterns(
        self, messages_with_timestamps: List[Tuple[str, str]]
    ) -> Dict[str, Any]:
            ...
            if token.dep_ == "nsubj" and token.pos_ in ["NOUN", "PROPN"]:
                ...
                if token.head.pos_ == "VERB":
                    ...
                    if len(predicate_parts) > 1:
                        full_concept = (
                            f"{subject_span.text} {' '.join(predicate_parts)}"
                        )
    ...
    def extract_topics(
        ...
        if quality_assessment["suitable_for_bertopic"]:
            ...
            bertopic_results = self._extract_topics_with_bertopic(
                messages, min_topic_size, nr_topics
            )
        else:
            logger.info(
                "Messages not suitable for BERTopic - using Discord-optimized approach"
            )
            bertopic_results = self._extract_topics_discord_optimized(
                messages, quality_assessment
            )
    ...
    def _assess_message_quality(self, messages: List[str]) -> Dict[str, Any]:
        ...
        for msg in messages:
            ...
            mention_count = (
                msg.count("@") + msg.count("<:") + len([c for c in msg if ord(c) > 127])
            )
            ...
            domain_words = [
                w for w in meaningful_words if self._is_domain_specific_term(w)
            ]
        ...
        suitable_for_bertopic = (
            total_messages >= 20
            and substantive_pct >= 0.5  # Increased minimum
            and short_pct < 0.5  # Increased from 0.3
            and mention_heavy_pct  # Reduced from 0.7 (less tolerance for short messages)
            < 0.4
            and conversational_pct < 0.6  # Reduced from 0.6
            and long_pct  # New criterion: less than 60% conversational
            >= 0.2  # At least 20% long messages for depth
        )
    ...
    def _extract_topics_with_bertopic(
        self, messages: List[str], min_topic_size: int, nr_topics: Optional[int]
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"Extracting topics from {len(cleaned_messages)} messages using BERTopic"
            )
            ...
            umap_model = self._umap_model(
                n_neighbors=min(
                    20, max(5, n_messages // 4)
                ),  # Larger neighborhoods for stability
                n_components=min(
                    10, max(5, n_messages // 10)
                ),  # More dimensions for complex topics
            ...
            hdbscan_model = self._hdbscan_model(
                min_cluster_size=max(
                    5, min(n_messages // 6, 15)
                ),  # Larger clusters for coherence
                min_samples=max(
                    3, min(n_messages // 10, 8)
            ...
            for _, row in topic_info.iterrows():
                ...
                filtered_keywords = [
                    (word, score) for word, score in topic_words[:10] if word.strip()
                ]
                ...
                topic_result = {
                    ...
                    "keyword_scores": [
                        round(score, 3) for word, score in filtered_keywords
                    ],
            ...
            logger.info(
                f"Successfully extracted {len(bertopic_results)} high-quality BERTopic topics"
            )
    ...
    def _extract_topics_discord_optimized(
        self, messages: List[str], quality_assessment: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        for message in messages:
            ...
            for i in range(len(words) - 1):
                if len(words[i]) > 3 and len(words[i + 1]) > 3:
                    ...
                    if not self._is_generic_phrase(
                        phrase
                    ) and self._is_meaningful_phrase(phrase):
            ...
            for word in words:
                if (
                    len(word) > 4
                    and self._is_domain_specific_term(word)
                    and not self._is_generic_word(word)
                ):
        ...
        help_topics = [
            topic for topic, count in questions_asked.most_common(3) if count >= 2
        ]
        if help_topics:
            topics.append(
                {
                    ...
                    "relevance_score": round(
                        sum(questions_asked.values()) / len(messages), 3
                    ),
    ...
    def _is_meaningful_phrase(self, phrase: str) -> bool:
        ...
        meaningful_count = sum(
            ...
            or len(word) > 5
            or word
            in ["meeting", "session", "team", "project", "work", "time", "schedule"]
        )
    ...
    def _is_domain_specific_term(self, word: str) -> bool:
        ...
        return (
            word_lower in tech_terms
            or word_lower in business_terms
            or word_lower in industry_terms
            or word_lower in tool_terms
        )
    ...
    def _post_process_topics(
        ...
        if not filtered_topics and topics:
            logger.info(
                "No topics passed strict quality filter, using moderately relaxed criteria"
            )
            for topic in topics:
                ...
                meaningful_keywords = [
                    ...
                    if (
                        len(kw) > 3
                        and self._is_domain_specific_term(kw)
                        and not self._is_generic_word(kw)
                    )
                ]

                if (
                    topic["frequency"] >= 3
                    and topic["relevance_score"] >= 0.015
                    and len(meaningful_keywords) >= 1  # At least 1.5% of messages
                    and topic.get(  # At least 1 domain-specific term
                        "coherence_score", 0
                    )
                    >= 0.05
                ):
        ...
        for topic in merged_topics:
            ...
            if (
                topic["frequency"] >= 3
                and topic["relevance_score"] >= 0.015
                and len(topic.get("keywords", [])) >= 1
            ):
                final_topics.append(topic)

        logger.info(
            f"Post-processing: {len(topics)} -> {len(filtered_topics)} -> {len(merged_topics)} -> {len(final_topics)} topics"
        )
        return final_topics
    ...
    def _is_high_quality_topic(self, topic: Dict[str, Any]) -> bool:
        ...
        for keyword in keywords[:5]:  # Check top 5 keywords
            if (
                len(keyword) > 2
                and keyword.lower() not in noise_words
                and not keyword.isdigit()
                and keyword.isalpha()
                and not self._is_generic_word(keyword)
            ):
    ...
    def _are_topics_similar(
        self, topic1: Dict[str, Any], topic2: Dict[str, Any]
    ) -> bool:
    ...
    def _merge_topic_group(self, topics: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        total_relevance = sum(
            topic["relevance_score"] * topic["frequency"] for topic in topics
        )
        ...
        sorted_keywords = sorted(
            keyword_score_map.items(), key=lambda x: x[1], reverse=True
        )
    ...
    def _clean_messages(self, messages: List[str]) -> List[str]:
        ...
        for message in messages:
            ...
            text = re.sub(
                r"\b(just|well|dont|cant|wont|isnt|arent|wasnt|werent|hasnt|havent|hadnt|didnt|doesnt|wouldnt|couldnt|shouldnt)\b",
                "",
                text,
                flags=re.IGNORECASE,
            )
    ...
    def _get_representative_docs(
        self, topic_id: int, topics: List[int], documents: List[str], max_docs: int = 3
    ) -> List[str]:
    ...
    def _generate_topic_name(
        self, topic_words: List[Tuple], representative_docs: List[str]
    ) -> str:
    ...
    def _create_semantic_topic_name(
        self, words: List[str], representative_docs: List[str]
    ) -> str:
        ...
        for category, patterns in business_patterns.items():
            if any(word.lower() in patterns for word in words[:5]):
                ...
                if relevant_words:
                    return (
                        f"{category.title()}: {' & '.join(relevant_words[:2]).title()}"
                    )
        ...
        try:
            ...
            if hasattr(self, "data_facade") and self.data_facade:
                ...
                try:
                    if channel_name:
                        message_data = (
                            self.data_facade.message_repository.get_channel_messages(
                                channel_name, days_back=days_back, limit=1000
                            )
                        )
                    else:
                        message_data = (
                            self.data_facade.message_repository.get_recent_messages(
                    ...
                    return AnalysisErrorResponse(
                        error=f"Failed to fetch messages: {e}", plugin="TopicAnalyzer"
                    )
            else:
                # Fallback for direct usage without data facade
                logger.warning(
                    "TopicAnalyzer used without data facade - limited functionality"
                )
                messages = []
                messages_with_timestamps = []
            ...
            logger.info(
                f"Analyzing {len(messages)} messages with hybrid BERTopic + spaCy approach"
            )
            ...
            if domain_analysis:
                ...
                if "technical_terms" in domain_analysis:
                    response._technical_terms_count = len(
                        domain_analysis["technical_terms"]
                    )
                if "emerging_topics" in domain_analysis:
                    response._emerging_topics_count = len(
                        domain_analysis["emerging_topics"]
                    )
            ...
            return AnalysisErrorResponse(
                error=f"Analysis failed: {e}", plugin="TopicAnalyzer"
            )
