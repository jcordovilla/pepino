...
class AnalysisDataFacade:
    ...
    def get_cross_channel_summary(
        self, days_back: int = 7, limit: int = 10
    ) -> List[Dict]:
        """
                ...
                try:
                    ...
                    if channel_messages:
                        ...
                        if message_count > 0:
                            ...
                            total_length = sum(
                                len(msg.get("content", "")) for msg in human_messages
                            )
                            avg_length = (
                                total_length / message_count if message_count > 0 else 0
                            )

                            # Get unique human users
                            unique_human_users = len(
                                set(
                                    msg.get("username")
                                    for msg in human_messages
                                    if msg.get("username")
                                    and not msg.get("author_is_bot", False)
                                )
                            )
...
def get_analysis_data_facade(
    db_manager: Optional[DatabaseManager] = None, base_filter: Optional[str] = None
) -> AnalysisDataFacade:
    """
