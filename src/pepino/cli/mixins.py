...
class CLITemplateMixin:
    ...
    def create_template_engine(self, data_facade=None):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "NLP service not available in CLI, templates will have limited NLP capabilities"
                )
                nlp_service = None
    ...
    def render_cli_template(
        self, template_name: str, data: Dict[str, Any], data_facade=None, messages=None
    ) -> str:
        """
        ...
        try:
            ...
            return template_engine.render_template(
                f"outputs/cli/{template_name}", messages=messages, **data
            )
    ...
    def handle_output(
        ...
        if output_format == "text" and not output_file and template_name:
            ...
            formatted_output = self.render_cli_template(
                template_name, data, data_facade, messages
            )
    ...
    def _write_structured_output(
        self, data: Dict[str, Any], output_file: Optional[str], output_format: str
    ):
    ...
    def show_template_success(
        self, operation: str, template_name: str = None, exec_time: float = None
    ):
...
class CLIAnalysisMixin(CLITemplateMixin):
    ...
    def handle_analysis_result(
        ...
        self.handle_output(
            result, output_file, output_format, template_name, data_facade, messages
        )
