    ...
    try:
        with get_database_manager(db_path) as db_manager:
            ...
            if user:
                ...
                result = user_analyzer.analyze(username=user, include_patterns=True)
                if result:
            ...
            if channel:
                ...
                if result:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Could not get total human members for channel {channel}: {e}"
                        )
                    ...
                    if result.top_users and len(result.top_users) >= 5:
                        top_5_messages = sum(
                            user.message_count for user in result.top_users[:5]
                        )
                    ...
                    try:
                        ...
                        all_users = (
                            data_facade.channel_repository.get_channel_user_activity(
                                channel, days=None, limit=100
                            )
                        )
                        recent_users = (
                            data_facade.channel_repository.get_channel_user_activity(
                                channel, days=30, limit=100
                            )
                        )
                        ...
                        logger.warning(
                            f"Could not calculate lost interest for channel {channel}: {e}"
                        )
                    ...
                    if result.engagement_metrics:
                        ...
                        if reaction_rate > 80:
                            engagement_summary = (
                                f"High ({reaction_rate:.0f}% reaction rate)"
                            )
                            ...
                            engagement_summary = (
                                f"Low ({reaction_rate:.0f}% reaction rate)"
                            )
                    ...
                    try:
                        ...
                        current_messages = (
                            data_facade.message_repository.get_channel_messages(
                                channel, days_back=7
                            )
                        )
                        previous_messages = (
                            data_facade.message_repository.get_channel_messages(
                                channel, days_back=14, limit=len(current_messages) * 2
                            )
                        )
                        ...
                        logger.warning(
                            f"Could not calculate trend for channel {channel}: {e}"
                        )
                    ...
                    if result.statistics.bot_messages > 0:
                        bot_percentage = (
                            result.statistics.bot_messages
                            / result.statistics.total_messages
                        ) * 100
                        if (
                            result.statistics.bot_messages
                            > result.statistics.human_messages
                        ):
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Could not calculate recent activity for channel {channel}: {e}"
                        )
    ...
    try:
        ...
        with get_database_manager(db_path) as db_manager:
            ...
            if user_name:
                ...
                messages = data_facade.message_repository.get_user_messages(
                    user_name, days_back=days_back
                )
            elif channel_name:
                # Get messages from specific channel
                messages = data_facade.message_repository.get_channel_messages(
                    channel_name, days_back=days_back
                )
            else:
                # Get recent messages
                messages = data_facade.message_repository.get_recent_messages(
            ...
            positive_count = sum(
                1 for s in sentiment_results if s["sentiment"] == "positive"
            )
            negative_count = sum(
                1 for s in sentiment_results if s["sentiment"] == "negative"
            )
            neutral_count = sum(
                1 for s in sentiment_results if s["sentiment"] == "neutral"
            )
            ...
            return {
                ...
                "positive_percentage": (positive_count / len(sentiment_results)) * 100
                if sentiment_results
                else 0,
                "negative_percentage": (negative_count / len(sentiment_results)) * 100
                if sentiment_results
                else 0,
                "neutral_percentage": (neutral_count / len(sentiment_results)) * 100
                if sentiment_results
                else 0,
    ...
    try:
        ...
        if channel_name:
            with get_database_manager(db_path) as db_manager:
                ...
                channel = data_facade.channel_repository.get_channel_by_name(
                    channel_name
                )
                if channel:
...
def export_all_tables_async(db_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"tables": tables, "exported_tables": ["messages", "channels", "users"]}
    except Exception as e:
