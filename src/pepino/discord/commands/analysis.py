...
class AnalysisCommands(ComprehensiveCommandMixin, commands.Cog):
    ...
    def __init__(self, bot):
        ...
        logger.info(
            "AnalysisCommands initialized with template system and sync analyzers"
        )
    ...
    def _create_template_engine(self, data_facade):
        ...
        try:
            ...
            logger.warning(
                "NLP service not available, templates will have limited NLP capabilities"
            )
            nlp_service = None
        ...
        return TemplateEngine(
            analyzers=analyzers, data_facade=data_facade, nlp_service=nlp_service
        )
        ...
        try:
            ...
            if current:
                ...
                starts_with = [
                    ch
                    for ch in channels
                    if ch.lower().startswith(current_lower)
                    and ch.lower() != current_lower
                ]
                ...
                contains = [
                    ch
                    for ch in channels
                    if current_lower in ch.lower()
                    and not ch.lower().startswith(current_lower)
                ]
    ...
    async def user_autocomplete(
        ...
        try:
            ...
            with get_analysis_data_facade(base_filter=settings.base_filter) as facade:
                ...
                users_data = facade.user_repository.get_users_for_autocomplete(
                    limit=200
                )
            ...
            valid_users = [
                ...
                if user_data["author_name"] and user_data["author_name"].strip()
            ]
            ...
            if current:
                ...
                for user_data in valid_users:
                    ...
                    if author_match or display_match:
                        ...
                        priority = 0
                        if (
                            author_name.lower() == current_lower
                            or display_name.lower() == current_lower
                        ):
                            priority = 1  # Exact match
                        elif author_name.lower().startswith(
                            current_lower
                        ) or display_name.lower().startswith(current_lower):
    ...
    def _sync_user_analysis(
        self, username: str, days: Optional[int], include_semantic: bool
    ) -> str:
        ...
        with get_analysis_data_facade(base_filter=settings.base_filter) as facade:
            ...
            user_temporal_data = facade.user_repository.get_user_temporal_data(
                username, days
            )
            ...
            template_data = {
                ...
                "time_patterns": [
                    {"period": pattern.period, "message_count": pattern.message_count}
            ...
            return template_engine.render_template(
                "outputs/discord/user_analysis.md.j2", **template_data
            )
    ...
    def _sync_topics_analysis(self, channel_name: str, days: Optional[int]) -> str:
        ...
        with get_analysis_data_facade(base_filter=settings.base_filter) as facade:
            ...
            # Render template
            return template_engine.render_template(
                "outputs/discord/topic_analysis.md.j2", **template_data
            )
    ...
    def _sync_top_users(self, limit: int, days: Optional[int]) -> str:
        ...
        with get_analysis_data_facade(base_filter=settings.base_filter) as facade:
            ...
            # Render template
            return template_engine.render_template(
                "outputs/discord/top_users.md.j2", **template_data
            )
    ...
    def _sync_server_overview(self, days: Optional[int]) -> str:
        ...
        with get_analysis_data_facade(base_filter=settings.base_filter) as facade:
            ...
            try:
                ...
                raw_human_bot_stats = self._get_server_human_bot_statistics(
                    facade, days
                )
                ...
                # Get top channels
                top_channels = (
                    facade.channel_repository.get_top_channels_by_message_count(
                        limit=10, days=days
                    )
                )
                ...
                temporal_analysis = temporal_analyzer.analyze(
                    channel_name=None, days_back=days  # Server-wide
                )
                ...
                if (
                    hasattr(temporal_analysis, "patterns")
                    and temporal_analysis.patterns
                ):
                ...
                try:
                    ...
                    first_result = facade.message_repository.db_manager.execute_query(
                        first_message_query, fetch_one=True
                    )
                    last_result = facade.message_repository.db_manager.execute_query(
                        last_message_query, fetch_one=True
                    )
                ...
                template_data = {
                    ...
                    "server_stats": {
                        ...
                        "total_messages": raw_human_bot_stats.get("human_messages", 0)
                        + raw_human_bot_stats.get("bot_messages", 0),
                        "total_users": raw_human_bot_stats.get("unique_human_users", 0)
                        + raw_human_bot_stats.get("unique_bot_users", 0),
                        "total_channels": total_channels,  # Keep filtered for analytics
                        "messages_per_user": round(
                            (
                                raw_human_bot_stats.get("human_messages", 0)
                                + raw_human_bot_stats.get("bot_messages", 0)
                            )
                            / max(
                                raw_human_bot_stats.get("unique_human_users", 0)
                                + raw_human_bot_stats.get("unique_bot_users", 0),
                                1,
                            ),
                            1,
                        ...
                        "messages_per_channel": round(
                            total_messages / total_channels, 1
                        )
                        if total_channels > 0
                        else 0,  # Use filtered for analytics
                        ...
                        "unique_human_users": raw_human_bot_stats.get(
                            "unique_human_users", 0
                        ),
                        "unique_bot_users": raw_human_bot_stats.get(
                            "unique_bot_users", 0
                        ),
                        "human_percentage": raw_human_bot_stats.get(
                            "human_percentage", 0
                        ),
                    ...
                    if hasattr(temporal_analysis, "patterns")
                    else None,
                ...
                # Render template
                return template_engine.render_template(
                    "outputs/discord/server_overview.md.j2", **template_data
                )
    ...
    def _get_server_human_bot_statistics(
        self, facade, days: Optional[int]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            result = facade.message_repository.db_manager.execute_query(
            ...
            human_percentage = (
                (human_messages / total_messages * 100) if total_messages > 0 else 0
            )
            bot_percentage = (
                (bot_messages / total_messages * 100) if total_messages > 0 else 0
            )
    ...
    def _get_server_temporal_data(self, facade, days: Optional[int]) -> Dict[str, Any]:
        ...
        try:
            ...
            if analysis_days:
                date_condition = (
                    f"timestamp >= datetime('now', '-{analysis_days} days')"
                )
            else:
            ...
            return {"activity_by_day": activity_by_day}

        except Exception as e:
            logger.error(f"Failed to get server temporal data: {e}")
            return {"activity_by_day": []}

    def _get_server_engagement_metrics(
        self, facade, days: Optional[int]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if settings.base_filter:
                query = query.replace(
                    "WHERE content IS NOT NULL",
                    f"WHERE content IS NOT NULL AND {settings.base_filter}",
                )
            ...
            human_replies_per_post = (
                round(human_replies / human_original_posts, 2)
                if human_original_posts > 0
                else 0
            )
            human_reaction_rate = (
                round((human_posts_with_reactions / human_original_posts) * 100, 1)
                if human_original_posts > 0
                else 0
            )
    ...
    def _get_server_topic_analysis(self, facade, days: Optional[int]) -> Dict[str, Any]:
        ...
        try:
            ...
            results = facade.message_repository.db_manager.execute_query(
                query, tuple(params)
            )
    ...
    def _sync_analyze_single_channel(self, channel_name: str, **analysis_params) -> str:
        ...
        with get_analysis_data_facade(base_filter=settings.base_filter) as facade:
            ...
            analysis_result = channel_analyzer.analyze(
                channel_name=channel_name, **analyzer_params
            )

            # Debug: Raw analysis data
            logger.debug(
                f"Raw analysis_result.statistics: {getattr(analysis_result, 'statistics', None)}"
            )
            logger.debug(
                f"Raw analysis_result.top_users: {getattr(analysis_result, 'top_users', None)}"
            )
            logger.debug(
                f"Raw analysis_result.engagement_metrics: {getattr(analysis_result, 'engagement_metrics', None)}"
            )
            ...
            try:
                total_human_members = (
                    facade.channel_repository.get_channel_human_member_count(
                        channel_name
                    )
                )
            ...
            try:
                all_users = facade.channel_repository.get_channel_user_activity(
                    channel_name, days=None, limit=100
                )
                logger.debug(f"Raw all_users (all time): {all_users}")
                recent_users = facade.channel_repository.get_channel_user_activity(
            ...
            # Debug: Raw messages
            try:
                all_messages = facade.message_repository.get_channel_messages(
                    channel_name, days_back=None, limit=100
                )
                logger.debug(f"Raw all_messages (all time, up to 100): {all_messages}")
                recent_messages_debug = facade.message_repository.get_channel_messages(
                    channel_name, days_back=7, limit=100
                )
                logger.debug(
                    f"Raw recent_messages (7d, up to 100): {recent_messages_debug}"
                )
            except Exception as e:
            ...
            if analysis_result.top_users:
                ...
                top_messages = sum(
                    user.message_count
                    for user in analysis_result.top_users[:num_contributors]
                )
                total_messages = analysis_result.statistics.total_messages
                logger.debug(
                    f"Participation calculation: {num_contributors} contributors, {top_messages} messages, {total_messages} total"
                )
                ...
                if inactive_users:
                        ...
                        if user["last_message"]:
                            try:
                                last_msg_date = datetime.fromisoformat(
                                    user["last_message"].replace("Z", "+00:00")
                                )
                    ...
                    if lost_interest_users:
                        ...
                        lost_interest_users.sort(
                            key=lambda x: (x["days_inactive"], x["message_count"]),
                            reverse=True,
                        )
            ...
            if analysis_result.engagement_metrics:
                ...
                if reaction_rate > 80:
                    ...
                    engagement_summary = (
                        f"Moderate ({reaction_rate:.0f}% reaction rate)"
                    )
                else:
            ...
            try:
                ...
                current_messages = facade.message_repository.get_channel_messages(
                    channel_name, days_back=7
                )
                previous_messages = facade.message_repository.get_channel_messages(
                    channel_name, days_back=14, limit=len(current_messages) * 2
                )
            ...
            if analysis_result.statistics.bot_messages > 0:
                bot_percentage = (
                    analysis_result.statistics.bot_messages
                    / analysis_result.statistics.total_messages
                ) * 100
                logger.debug(
                    f"Bot activity calculation: {analysis_result.statistics.bot_messages} bot messages, {analysis_result.statistics.human_messages} human messages, {bot_percentage:.1f}%"
                )

                if (
                    analysis_result.statistics.bot_messages
                    > analysis_result.statistics.human_messages
                ):
            ...
            try:
                ...
                logger.warning(
                    f"Could not calculate response time for channel {channel_name}: {e}"
                )

            # Calculate recent activity summary
            recent_activity_summary = None
            try:
                recent_messages = facade.message_repository.get_channel_messages(
                    channel_name, days_back=7
                )
                if recent_messages:
                    human_recent = len(
                    ...
                    previous_messages = facade.message_repository.get_channel_messages(
                        channel_name, days_back=14, limit=len(recent_messages) * 2
                    )
                    if previous_messages:
                        ...
                        if human_previous > 0:
                            change_percent = (
                                (human_recent - human_previous) / human_previous
                            ) * 100
                            if change_percent > 0:
                            ...
                            recent_activity_summary = (
                                f"{human_recent} messages in last 7 days"
                            )
            except Exception as e:
                logger.warning(
            ...
            try:
                ...
                recent_messages_data = facade.message_repository.get_channel_messages(
                    channel_name, days_back=analyzer_params.get("days", 7), limit=200
                )
                ...
                if recent_messages_data:
                    ...
                    for msg in recent_messages_data:
                        ...
                        ):  # Only analyze substantial messages
                            ...
                            frequent_terms.append(
                                {
                                    ...
                                    "frequency": count,
                                    "relevance": min(
                                        count / len(recent_messages_data), 1.0
                                    ),  # Normalize relevance
                                }
                            )

                    logger.debug(
                        f"Found {len(frequent_terms)} frequent terms for channel {channel_name}"
                    )
                else:
                    logger.debug(
                        f"No recent messages for NLP analysis in channel {channel_name}"
                    )
            except Exception as e:
                logger.warning(
                    f"Could not perform NLP analysis for channel {channel_name}: {e}"
                )
            ...
            try:
                ...
                messages_data = facade.message_repository.get_channel_messages(
                    channel_name, days_back=7, limit=100
                )
                if messages_data:
                    recent_messages = [
            ...
            if temporal_data:
                try:
                    chart_path = self._generate_daily_messages_chart(
                        temporal_data, analysis_params
                    )
                    if not chart_path:
    ...
    def _sync_analyze_all_channels_summary(self, **analysis_params):
        ...
        with get_analysis_data_facade(base_filter=settings.base_filter) as facade:
            ...
            channels_summary = facade.get_cross_channel_summary(
                days_back=analysis_params.get("days_back", 7), limit=10
            )
            ...
            avg_messages_per_channel = (
                total_messages / total_channels if total_channels > 0 else 0
            )
            ...
            template_data = {
                ...
                "trends": {
                    ...
                    "avg_messages_per_period": sum(
                        [d.message_count for d in analysis_result.temporal_data]
                    )
                    / len(analysis_result.temporal_data)
                    if analysis_result.temporal_data
                    else 0,
                    "max_messages_in_period": max(
                        [d.message_count for d in analysis_result.temporal_data]
                    )
                    if analysis_result.temporal_data
                    else 0,
                ...
                "days": analysis_params.get("days_back", 7),
                "peak_analysis": {"peak_hour": None, "peak_day": None},
                # Add cross-channel summary data
                "cross_channel_stats": {
                    "total_channels_analyzed": total_channels,
                    "total_messages": total_messages,
                    "avg_messages_per_channel": avg_messages_per_channel,
                    "most_active_channel": channels_summary[0]["name"]
                    if channels_summary
                    else "None",
                    "most_active_count": channels_summary[0]["message_count"]
                    if channels_summary
                    else 0,
                },
            }
            ...
            if analysis_result.temporal_data:
                try:
                    chart_path = self._generate_daily_messages_chart(
                        analysis_result.temporal_data, analysis_params
                    )
                    if not chart_path:
                        logger.warning("Chart generation returned None")
                except Exception as e:
            ...
            if not chart_path and channels_summary:
                try:
                    chart_path = self._generate_channels_summary_chart(
                        channels_summary, analysis_params
                    )
                except Exception as e:
...
"""
                ...
                text_result = template_engine.render_template(
                    "outputs/discord/activity_trends.md.j2", **template_data
                )
    ...
    def _generate_daily_messages_chart(self, temporal_data, analysis_params):
        ...
        try:
            ...
            for data_point in temporal_data:
                ...
                if hasattr(data_point, "period") and hasattr(
                    data_point, "message_count"
                ):
                    ...
                    try:
                        ...
                        continue
                elif hasattr(data_point, "date") and hasattr(
                    data_point, "message_count"
                ):
                    try:
                    ...
                    try:
                        date_str = (
                            data_point.get("period")
                            or data_point.get("date")
                            or data_point.get("timestamp")
                        )
                        count = data_point.get("message_count") or data_point.get(
                            "count"
                        )
            ...
            if len(message_counts) >= 7:
                ...
                for i in range(len(message_counts)):
                    if i < window_size - 1:
                        ...
                        # Use 7-day window
                        moving_avg.append(
                            np.mean(message_counts[i - window_size + 1 : i + 1])
                        )
            ...
            ax.set_title(
                f"Daily Message Activity ({analysis_params.get('start_date', '')} to {analysis_params.get('end_date', '')})"
            )
            ax.legend()
    ...
    def _generate_channels_summary_chart(self, channels_summary, analysis_params):
        ...
        try:
            ...
            ax.set_ylabel("Number of Messages")
            ax.set_title(
                f"Channel Activity Summary ({analysis_params.get('start_date', '')} to {analysis_params.get('end_date', '')})"
            )
            ax.legend()
            ax.grid(True, alpha=0.3)
    ...
    @app_commands.command(
        name="pepino_help",
        description="Show available Pepino commands and their descriptions",
    )
    ...
    @app_commands.command(
        name="pepino_channel_analytics",
        description="Comprehensive channel analysis with multiple analysis types",
    )
        ...
        try:
            ...
            if analysis_type in ["topics"] and not channel_name:
                if isinstance(interaction.channel, discord.DMChannel):
                    await interaction.response.send_message(
                        f"❌ Please specify a channel name when using {analysis_type} analysis in DMs."
                    )
                    return
                channel_name = interaction.channel.name

            logger.info(
                f"Pepino channel analytics command called: type={analysis_type}, channel={channel_name}, days={days}, end_date={end_date}"
            )
            ...
            if analysis_type == "overview":
                await self._handle_channel_overview(
                    interaction, channel_name, days, end_date
                )
    ...
    async def _handle_channel_overview(
        self,
        interaction: discord.Interaction,
        ...
        if end_date:
            try:
                ...
                await interaction.followup.send(
                    "❌ Invalid date format. Please use YYYY-MM-DD format. Example: 2024-12-25"
                )
                return
        else:
        ...
        if days is not None:
            ...
            if days <= 0:
                await interaction.followup.send(
                    f"❌ Days must be a positive number. You provided: {days}"
                )
                return
            ...
            analysis_params.update(
                {"start_date": None, "end_date": None, "days_back": None}
            )
            ...
            if chart_path and os.path.exists(chart_path):
                ...
                if len(text_result) <= max_length_with_file:
                    ...
                    await interaction.followup.send(
                        text_result,
                        file=discord.File(
                            chart_path, filename="daily_activity_chart.png"
                        ),
                    )
                else:
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not remove temporary chart file {chart_path}: {e}"
                    )
            else:
    ...
    async def _handle_channel_topics(
        self, interaction: discord.Interaction, channel_name: str, days: Optional[int]
    ):
        ...
        await interaction.followup.send(
            f"🔍 Analyzing topics in **{channel_name}** ({time_desc})... This may take a moment."
        )

        # Execute sync operation in thread pool with performance tracking
        result, exec_time = await self.execute_tracked_sync_operation(
            "topics_analysis", self._sync_topics_analysis, channel_name, days
        )
        ...
        if result:
            await self._send_long_message_slash(interaction, result)
            await interaction.followup.send(
                f"✅ Topics analysis completed in {exec_time:.2f}s"
            )
        else:
            await interaction.followup.send(
                f"❌ No topic data found for channel **{channel_name}**"
            )

    @app_commands.command(
        name="pepino_user_analytics",
        description="Analyze a specific user's activity and behavior patterns",
    )
    @app_commands.describe(
        ...
        try:
            ...
            if result:
                ...
                try:
                    ...
                    settings = Settings()
                    with get_analysis_data_facade(
                        base_filter=settings.base_filter
                    ) as facade:
                        temporal_data = facade.user_repository.get_user_temporal_data(
                            username, days
                        )

                        if temporal_data and temporal_data.get("activity_by_day"):
                            analysis_params = {"days": days or "all time"}
                            chart_path = self._generate_user_activity_chart(
                                username, temporal_data, analysis_params
                            )
                            ...
                            if chart_path:
                                try:
                                    ...
                                    chart_file = discord.File(
                                        chart_path,
                                        filename=f"user_activity_{username}.png",
                                    )
                                    await interaction.followup.send(
                                    ...
                                    logger.error(
                                        f"Failed to send user activity chart: {e}"
                                    )
                                    await interaction.followup.send(
                                        "⚠️ Chart generated but failed to send"
                                    )
                            else:
                                logger.warning("User activity chart generation failed")
                        else:
                            logger.warning(
                                f"No temporal data available for user {username} chart"
                            )
                ...
                await interaction.followup.send(
                    f"✅ User analysis completed in {exec_time:.2f}s"
                )
            else:
                await interaction.followup.send(
                    f"❌ No data found for user '{username}'"
                )
    ...
    @app_commands.command(
        name="pepino_server_analytics",
        description="Server-wide analysis including top users and overall statistics",
    )
    @app_commands.describe(
    ...
    async def pepino_server_analytics(
        ...
        try:
            ...
            logger.info(
                f"Pepino server analytics command called: type={analysis_type}, limit={limit}, days={days}"
            )
    ...
    async def _handle_server_top_users(
        self, interaction: discord.Interaction, limit: int, days: Optional[int]
    ):
        """Handle server top users analysis (equivalent to top_users)"""
        # Execute sync operation in thread pool with performance tracking
        result, exec_time = await self.execute_tracked_sync_operation(
            "top_users", self._sync_top_users, limit, days
        )
    ...
    async def _handle_server_overview(
        self, interaction: discord.Interaction, days: Optional[int]
    ):
        """Handle server overview analysis (equivalent to server_overview)"""
        # Send initial response
        time_desc = f"last {days} days" if days is not None else "all time"
        await interaction.followup.send(
            f"🔍 Generating server overview analysis ({time_desc})... This may take a moment."
        )

        # Execute sync operation in thread pool with performance tracking
        result, exec_time = await self.execute_tracked_sync_operation(
            "server_overview", self._sync_server_overview, days
        )
        ...
        if result:
            ...
            try:
                ...
                with get_analysis_data_facade(
                    ...
                    if temporal_data and temporal_data.get("activity_by_day"):
                        analysis_params = {"days": days}
                        chart_path = self._generate_server_overview_chart(
                            temporal_data, analysis_params
                        )
                        ...
                        if chart_path:
                            try:
                                ...
                                chart_file = discord.File(
                                    chart_path, filename="server_overview_chart.png"
                                )
                                await interaction.followup.send(
                                    "📊 **Server Activity Chart:**", file=chart_file
                                )
                                ...
                                logger.error(
                                    f"Failed to send server overview chart: {e}"
                                )
                                await interaction.followup.send(
                                    "⚠️ Chart generated but failed to send"
                                )
                        ...
                        logger.warning(
                            "No temporal data available for server overview chart"
                        )
            ...
            # Send completion message
            await interaction.followup.send(
                f"✅ Server overview analysis completed in {exec_time:.2f}s"
            )
        else:
    ...
    @app_commands.command(
        name="pepino_lists",
        description="List available channels and users for analysis",
    )
    @app_commands.describe(
        list_type="Type of list to show", limit="Number of results to show (optional)"
    )
    async def pepino_lists(
        ...
        try:
            ...
            if not interaction.response.is_done():
                await interaction.response.send_message(
                    f"❌ Failed to fetch {list_type}: {str(e)}"
                )
            else:
                await interaction.followup.send(
                    f"❌ Failed to fetch {list_type}: {str(e)}"
    ...
    async def _handle_list_channels(
        self, interaction: discord.Interaction, limit: Optional[int]
    ):
        ...
        channels, exec_time = await self.execute_tracked_sync_operation(
            "list_channels", self._sync_get_available_channels
        )
        ...
        if channels:
            ...
            if limit and limit > 0:
                ...
                channel_list = "\n".join(
                    [f"• **{channel}**" for channel in display_channels]
                )
                ...
                await self._send_paginated_channel_list(
                    interaction, channels, exec_time
                )
        else:
            await interaction.followup.send("❌ No channels found in database")

    async def _send_paginated_channel_list(
        self, interaction: discord.Interaction, channels: List[str], exec_time: float
    ):
        ...
        for channel in channels:
            ...
            if (
                current_length + len(channel_line) > max_chars_per_embed
                and current_page_channels
            ):
                # Start a new page
        ...
        for page_num, page_channels in enumerate(channels_per_page, 1):
            ...
            if total_pages == 1:
                ...
                title = "📺 Available Channels for Analysis"
                footer_text = (
                    f"Total: {len(channels)} channels • Retrieved in {exec_time:.2f}s"
                )
            else:
                # Multiple pages
                title = (
                    f"📺 Available Channels for Analysis (Page {page_num}/{total_pages})"
                )
                footer_text = f"Showing {len(page_channels)} channels on this page • Total: {len(channels)} channels • Retrieved in {exec_time:.2f}s"

            embed = discord.Embed(
                title=title, description=channel_list, color=discord.Color.blue()
            )
            embed.set_footer(text=footer_text)
    ...
    async def _handle_list_users(
        ...
        users, exec_time = await self.execute_tracked_sync_operation(
            "list_users", self._sync_get_available_users
        )
    ...
    async def _send_paginated_user_list(
        self, interaction: discord.Interaction, users: List[str], exec_time: float
    ):
        ...
        for user in users:
            user_line = f"• **{user}**\n"
            if (
                current_length + len(user_line) > max_chars_per_embed
                and current_page_users
            ):
                # Start a new page
        ...
        for page_num, page_users in enumerate(users_per_page, 1):
            ...
            if total_pages == 1:
                ...
                title = "👥 Available Users for Analysis"
                footer_text = (
                    f"Total: {len(users)} users • Retrieved in {exec_time:.2f}s"
                )
            else:
                # Multiple pages
                title = (
                    f"👥 Available Users for Analysis (Page {page_num}/{total_pages})"
                )
                footer_text = f"Showing {len(page_users)} users on this page • Total: {len(users)} users • Retrieved in {exec_time:.2f}s"

            embed = discord.Embed(
                title=title, description=user_list, color=discord.Color.green()
            )
            embed.set_footer(text=footer_text)
        ...
        try:
            ...
            for item in activity_data:
                try:
                    ...
                    logger.warning(
                        f"Invalid date format in temporal data: {item}, error: {e}"
                    )
                    continue

            if not dates or not message_counts:
                logger.warning(
                    "No valid temporal data for server overview chart generation"
                )
                return None
            ...
            logger.warning(
                "Matplotlib not available for server overview chart generation"
            )
            return None
        except Exception as e:
            logger.error(f"Server overview chart generation failed: {e}")
    ...
    def _generate_user_activity_chart(
        self, username: str, temporal_data, analysis_params
    ):
        ...
        try:
            ...
            if not dates or not message_counts:
                logger.warning(
                    f"No valid temporal data for user {username} chart generation"
                )
                return None
            ...
            ax.set_ylabel("Number of Messages")
            time_desc = (
                f"{analysis_params.get('days')} days"
                if analysis_params.get("days")
                else "all time"
            )
            ...
            logger.warning(
                "Matplotlib not available for user activity chart generation"
            )
            return None
        except Exception as e:
            logger.error(f"User activity chart generation failed: {e}")
