...
class ThreadedCommandMixin:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.thread_pool = ThreadPoolExecutor(
            max_workers=5, thread_name_prefix="discord_cmd"
        )
    ...
    async def run_sync_in_thread(
        self, sync_function: Callable, *args, timeout: float = 30.0, **kwargs
    ) -> Any:
        """
        ...
        logger.debug(
            f"Starting sync operation {operation_id}: {sync_function.__name__}"
        )
        ...
        try:
            ...
            result = await asyncio.wait_for(
                loop.run_in_executor(
                    self.thread_pool, lambda: sync_function(*args, **kwargs)
                ),
                timeout=timeout,
            )
    ...
    async def run_multiple_sync_in_thread(
        self, operations: Dict[str, tuple], timeout: float = 30.0
    ) -> Dict[str, Any]:
        """
    ...
    async def cleanup_thread_pool(self):
        ...
        try:
            await asyncio.wait_for(self._wait_for_operations_complete(), timeout=10.0)
        except asyncio.TimeoutError:
    ...
    async def _wait_for_operations_complete(self):
        ...
        while self._active_operations > 0:
            logger.debug(
                f"Waiting for {self._active_operations} operations to complete..."
            )
...
class PerformanceMonitorMixin:
    ...
    async def track_command_performance(
        self, command_name: str, operation: Callable, *args, **kwargs
    ) -> tuple[Any, float]:
        """
        ...
        try:
            ...
            if len(self._command_metrics[command_name]) > 100:
                self._command_metrics[command_name] = self._command_metrics[
                    command_name
                ][-100:]
            ...
            execution_time = time.time() - start_time
            logger.error(
                f"Command {command_name} failed after {execution_time:.2f}s: {e}"
            )
            raise
...
class ComprehensiveCommandMixin(ThreadedCommandMixin, PerformanceMonitorMixin):
    ...
    async def execute_tracked_sync_operation(
        ...
        **kwargs,
        ...
        return await self.track_command_performance(command_name, tracked_operation)
